package com.sinotech.view.form.data.column;
...
import com.sinotech.view.form.data.format.count.DecimalCountFormat;
import com.sinotech.view.form.data.format.count.ICountFormat;
import com.sinotech.view.form.data.format.count.NumberCountFormat;
import com.sinotech.view.form.data.format.count.StringCountFormat;
import com.sinotech.view.form.data.format.draw.FastTextDrawFormat;
import com.sinotech.view.form.data.format.draw.IDrawFormat;
import com.sinotech.view.form.data.format.draw.MultiLineDrawFormat;
import com.sinotech.view.form.data.format.draw.TextDrawFormat;
...
public class Column<T> implements Comparable<Column> {
    ...
    /**
     * 填充数据
     * @param objects 对象列表  object 合计对象
     * @return 返回需要合并的单元
     * @throws NoSuchFieldException
     * @throws IllegalAccessException
     */
    ...
    public void fillData(List<Object> objects,Object object) throws NoSuchFieldException, IllegalAccessException {
        ...
        // 合计处理
        if (object != null) {
            String[] totalFieldNames = fieldName.split("\\.");
            if (totalFieldNames.length > 0) {
                Field[] fields = new Field[totalFieldNames.length];
                Object child = object;
                for (int i = 0; i < totalFieldNames.length; i++) {
                    ...
                    if (fields[i] != null) {
                        ...
                        Class childClazz = child.getClass();
                        childField = childClazz.getDeclaredField(totalFieldNames[i]);
                        childField.setAccessible(true);
                        fields[i] = childField;
                    }
                    if (childField == null) {
            ...
            throw new TableException("the total date must be not null !");
        }
    }
    ...
    public Comparator<T> getComparator() {
        if (isAutoCount && TextUtils.isEmpty(bottomName)){
            return new Comparator<T>() {
                @Override
                public int compare(T t, T t1) {
                    return LetterUtils.getNumMax(t,t1);
                }
            };
        }else {
            if (comparator != null){
            ...
            return new Comparator<T>() {
                @Override
                public int compare(T t, T t1) {
                    return LetterUtils.getStringMax(t,t1);
                }
            };
        }
    }
     ...
     * 直接设置数据，不需要反射获取值
     */
    public void setTotalDatas(T totaldata) {
